  ...
  return (
    <div className='flex flex-col gap-5 py-5 px-3 h-[10rem] lg:h-[20rem]'>
      ...
      <div className='flex flex-col gap-3 lg:gap-5'>
        <h3 className='text-green font-semibold font-dm text-xl'>{props.accommodationName}</h3>
        <p className='text-grey italic text-xs lg:text-sm'>{props.location}</p>
        <p className='text-xs'>{props.description}</p>
        <button className='text-green text-xs font-dm uppercase rounded-full border border-green py-2 px-4'>Learn More</button>
      </div>
    </div>
  )
}
